...
# ----------------------------------------------------------------------------------------------------------------- #
ggplot(data = ag_results, aes(y = mse_linear, x = treatment_prevalence, group = method)) +  
  geom_line(aes(linetype = method,
                color = method)) +  
  geom_point(aes(shape = method,
                 color = method), size = 2) +
  facet_wrap(~ smd_covs + n_covs + sample_size,
             labeller = labeller(.multi_line = T,
                                 smd_covs = label_smd_covs,
                                 n_covs = label_n_covs,
