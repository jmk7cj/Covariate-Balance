...
#
# Date: 10/22/2021 (under construction)
#
...
estimate_ps <- function(d) {
  ...
  match_wr7 <- matchit(ps_form, data=d, link="linear.logit", method="nearest", 
                       estimand="ATT", replace=T, ratio=7)
  ...
  avg_mwr7_bal_smd <- mean(abs(bal.tab(match_wr7)$Balance[,3]))
  ...
  matchwr7_data <- match.data(match_wr7)
  # none for weighting
  ...
  est_att_mwr7 <- svydesign(ids=~1, weights =~ weights, data = matchwr7_data)
  ...
  est_ATT_mwr7 <- svyglm(reformulate(c("treat", paste0("x", seq_len(n_covs))), response = "y_observed"), est_att_mwr7, family=gaussian())
  ...
  att_est_mwr7 <- est_ATT_mwr7$coefficients["treat"]
  att_est_weight <- est_ATT_weight$coefficients["treat"]

  return(list(prop_treat, pop_ATE, pop_ATT, avg_unbal_smd, 
              avg_mwr1_bal_smd, avg_mwr3_bal_smd, avg_mwr5_bal_smd, 
              avg_mwr7_bal_smd, avg_wbal_smd,
              att_est_mwr1, att_est_mwr3, att_est_mwr5, 
              att_est_mwr7, att_est_weight))
}
# ----------------------------------------------------------------------------------------------------------------- #  
...
colnames(results)[(ncol(design_matrix)+1):ncol(results)] <- c("prop_treat", "pop_ATE", "pop_ATT","avg_unbal_smd", 
                                                              ...
                                                              "att_est_mwr1", "att_est_mwr3", "att_est_mwr5", 
                                                              "att_est_mwr7", "att_est_weight") #, 
                                                              #"bias_att_mwr1", "bias_att_mwr3", "bias_att_mwr5", 
                                                              #"bias_att_mwr7", "bias_att_weight")
...
results$mwr7_bal_smd_reduction <- (results$avg_unbal_smd - results$avg_mwr7_bal_smd) / results$avg_unbal_smd
...
results$bias_att_mwr7 <- (results$att_est_mwr7 - results$pop_ATT) / results$pop_ATT
...
ag_results <- aggregate(. ~ sample_size + treatment_effect + treatment_prevalence + n_covs + smd_covs + smd_y + r_square,
                       #data = results[,2:ncol(results)], FUN = mean)
                       data = results[,-1], FUN = mean)

ag_results <- ag_results[rep(seq_len(nrow(ag_results)), each=5), ]
ag_results$method <- rep(c("match_wr1", "match_wr3", "match_wr5", "match_wr7", "weight"), each=1)
...
ag_results$avg_bal <- ifelse(ag_results$method == "match_wr1", ag_results$avg_mwr1_bal_smd, 
                             ifelse(ag_results$method == "match_wr3", ag_results$avg_mwr3_bal_smd, 
                                    ifelse(ag_results$method == "match_wr5", ag_results$avg_mwr5_bal_smd, 
                                           ifelse(ag_results$method == "match_wr7", ag_results$avg_mwr7_bal_smd, 
...
ag_results$smd_reduction <- ifelse(ag_results$method == "match_wr1", ag_results$mwr1_bal_smd_reduction,
                                   ifelse(ag_results$method == "match_wr3", ag_results$mwr3_bal_smd_reduction,
                                          ifelse(ag_results$method == "match_wr5", ag_results$mwr5_bal_smd_reduction,
                                                 ifelse(ag_results$method == "match_wr7", ag_results$mwr7_bal_smd_reduction, 
...
ag_results$att_est <- ifelse(ag_results$method == "match_wr1", ag_results$att_est_mwr1,
                                   ifelse(ag_results$method == "match_wr3", ag_results$att_est_mwr3,
                                          ifelse(ag_results$method == "match_wr5", ag_results$att_est_mwr5,
                                                 ifelse(ag_results$method == "match_wr7", ag_results$att_est_mwr7, 
...
ag_results$bias_att <- ifelse(ag_results$method == "match_wr1", ag_results$bias_att_mwr1,
                                   ifelse(ag_results$method == "match_wr3", ag_results$bias_att_mwr3,
                                          ifelse(ag_results$method == "match_wr5", ag_results$bias_att_mwr5,
                                                 ifelse(ag_results$method == "match_wr7", ag_results$bias_att_mwr7, 
...
# Plot balance 
# ----------------------------------------------------------------------------------------------------------------- #  
label_method <- c("Matching with Replacement NN = 1",
                  "Matching with Replacement NN = 3",
                  "Matching with Replacement NN = 5",
                  "Weighting")
...
label_smd_covs <- c("0.2" = "Covariate Imbalance SMD = 0.2",
                    "0.5" = "Covariate Imbalance SMD = 0.5",
                    "0.8" = "Covariate Imbalance SMD = 0.8")

plot_data <- subset(ag_results, method != "match_wr7")
plot2 <- ggplot(data=plot_data, aes(y=(smd_reduction)*100, x=as.factor(treatment_prevalence), group=as.factor(method))) +
